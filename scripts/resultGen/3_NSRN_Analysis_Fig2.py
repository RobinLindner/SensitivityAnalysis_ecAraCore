...
## Output paths
fig_out_path = FIG_DIR / "enzyme_sensitivity_trajectories.png"
...
def main():
    ...
    for i in np.arange(1,11):
        res = k_means(X,n_clusters=i,random_state=5)
        inertia.append(res[2])
    plt.plot(np.arange(1,11),inertia)
    plt.title("Inertia by number of clusters.")
    plt.show(block=SHOW_FIGURES)
    t = k_means(X,n_clusters = 4,random_state=5)
    res = silhouette_score(X,t[1])
    print(f"The average silhouette score is {res}.")
    print()
    ...
    sns.scatterplot(x=X_embedded[:,0],y=X_embedded[:,1],hue=t[1]).set_title("tSNE embedding")
    plt.show(block=SHOW_FIGURES)
    ...
    gs = fig.add_gridspec(1, 3, width_ratios=[1, 0.1, 1.5]) # 4 rows, 2 columns
    gs_left = gs[0].subgridspec(2,1)
    ax1 = fig.add_subplot(gs_left[0])

    ax2 = fig.add_subplot(gs_left[1],sharex=ax1)

    # Right column: 4 plots, each taking 1 row
    gs_right= gs[2].subgridspec(4,1)
    ax3 = fig.add_subplot(gs_right[0])
    ax4 = fig.add_subplot(gs_right[1],sharex=ax3)
    ax5 = fig.add_subplot(gs_right[2],sharex=ax3)
    ax6 = fig.add_subplot(gs_right[3],sharex=ax3)

    ax=[ax1,ax2,ax3,ax4,ax5,ax6]

    labelmap = {0:"Continuous",
                1:"Spurious",
                2:"High",
                3:"Low"}
    ...
    group_reorder = {0:1,
                    ...
                    2:3,
                    3:2}
    group_assignment = np.zeros_like(res[1])
    for i in np.arange(0,4):
        group_assignment[res[1]==i] = group_reorder[i] 
    ...
    sc = ax[0].scatter(X_reduced[:,0],X_reduced[:,1],pointsizes,c=[colormap[c] for c in group_assignment],edgecolors=outline)

    ax[1].scatter(X_reduced[:,0],X_reduced[:,2],pointsizes,c=[colormap[c] for c in group_assignment],edgecolors=outline)

    rel_idx = np.where(outline != "none")[0]
    ax[0].scatter(X_reduced[rel_idx,0],X_reduced[rel_idx,1],np.array(pointsizes)[rel_idx],c=[colormap[c] for c in group_assignment[rel_idx]],edgecolors=outline[rel_idx])
    ax[1].scatter(X_reduced[rel_idx,0],X_reduced[rel_idx,2],np.array(pointsizes)[rel_idx],c=[colormap[c] for c in group_assignment[rel_idx]],edgecolors=outline[rel_idx])
    ...
    for i in np.arange(0,4):
        ...
        subset = full_df[group_assignment==i].rename_axis("Enzyme").reset_index()
        # Turn to long format
        ...
        h, l = ax[i+2].get_legend_handles_labels()
        ax[i+2].text(x=10,y=0.8,s = f"$n: {len(np.unique(subset["Enzyme"]))}$")
        ax[i+2].set_ybound(0,1)
    ...
    fig.text(0.41, 0.5, 'Normalized Sensitivity', 
            va='center', rotation=90,fontsize=13)
    fig.text(0,1, "A", fontweight="bold", 
            va='center',fontsize=14)
    fig.text(0.41,1, "B", fontweight="bold", 
            va='center',fontsize=14)
