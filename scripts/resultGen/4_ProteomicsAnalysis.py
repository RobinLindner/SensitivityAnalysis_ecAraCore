...
def main():
    ...
    print("== ROBUSTNESS ANALYSIS WITH PERTURBATION DATA ==")
    ...
    for acc in ["Bur.0","Pla.0"]:
        for temp in [17,27]:
            ...
            print(f"{acc} at {temp}")
            print(f"Mean: {data.mean()},\tMedian: {data.median()},\tSD: {data.std()},\tSE: {data.std()/np.sqrt(len(data))},\tMin: {data.min()},\tMax: {data.max()}")
    ...
    mean_sens_corr_wide = mean_sens_corr_df.pivot(index="Subsystem",columns=["Temperature","Accession"])
    ...
    mean_pred_corr_wide = mean_pred_corr_df.pivot(index="Subsystem",columns=["Temperature","Accession"])
    ...
    for acc in ["Bur.0","Pla.0"]:
        ...
        for temp in [17,27]:
            ...
            lab = ""
            axes[i,j].set_ylabel(f"{acc}:{temp}°C")
            j+=1
        i+=1
    ...
    fig.savefig(SUPP_FIG_DIR / "IsoenzymesFoundBySens.png")
    ...
    for ax in axes.flat:
        ...
        ax.set_xlabel("Isoenzymes in reaction")
    fig.legend(handles,labels,ncols=3,loc="upper center",bbox_to_anchor=(0.5,0))
    fig.savefig(SUPP_FIG_DIR / "IsoenzymesFoundBySensAtOther.png")
